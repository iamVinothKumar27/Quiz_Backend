...
passport.use(new GoogleStrategy({
    ...
    callbackURL: process.env.GOOGLE_CALLBACK_URL || `${CONFIG.SERVER_URL}/api/auth/google/callback`
  },
  async (accessToken, refreshToken, profile, done) => {
    try {
      ...
      user = new User({
        ...
        role: role,
        ssoProvider: 'google', // NEW FIELD
        ssoId: profile.id       // NEW FIELD
      });
...
isActive: {
  ...
  createdByUserId: {  // ADD THIS NEW FIELD
...
const verifyAdminOwnership = async (req, res, next) => {
  try {
    const { sessionId } = req.params;
    const { adminEmail } = req.body; // or from headers/token
...
app.put('/api/quiz-sessions/:sessionId', verifyAdminOwnership, async (req, res) => {
  // Session update logic here
});
...
app.get('/api/quiz-sessions', async (req, res) => {
  try {
    const { adminEmail } = req.query; // Pass admin email as query parameter
    ...
    const sessions = await QuizSession.find({ createdBy: adminEmail })
      .sort({ createdAt: -1 })
      .select('-__v');
...
// Create new quiz session - include admin info
app.post('/api/quiz-sessions', async (req, res) => {
  try {
    const { name, createdBy, adminUserId } = req.body; // Add adminUserId
    
    if (!name || !createdBy || !adminUserId) {
      return res.status(400).json({ 
        message: 'Name, createdBy, and adminUserId are required' 
      });
    }
    ...
    const newSession = new QuizSession({
      ...
      name,
      createdBy,
      createdByUserId: adminUserId, // Add this
      questions: [],
      isActive: false
    });
...
// Get results for admin's sessions only
app.get('/api/quiz-results/:sessionId', async (req, res) => {
  try {
    const sessionId = req.params.sessionId;
...
// SSO Routes (add after your existing routes)
...
app.get('/api/auth/google/callback',
  ...
  (req, res) => {
    ...
    delete userResponse.password;
    const fe = (CONFIG.FRONTEND_URL || '').replace(/\/$/, '');
    res.redirect(`${fe}/auth/success?user=${encodeURIComponent(JSON.stringify(userResponse))}`);
  }
);
